...
/// The Forwarder is the Swiss Army Knife of machine models. It implements the TestMessage instruction set.
...
///
/// See Also: [ChaosMonkeyDriver], [DaisyChainDriver]
...
impl ForwarderMutable {
    ...
    fn handle_action(&mut self, message: TestMessage, id: usize, sender: &mut MachineSender) {
        match message {
    ...
    fn handle_notification(&mut self, sender: &mut MachineSender) {
...
impl Forwarder {
    ...
    /// Get the count of received commands. The counter will be reset to 0 by this method.
...
impl Machine<TestMessage> for Forwarder {
    ...
    fn receive(&self, message: TestMessage, sender: &mut MachineSender) {
...
mod tests {
    ...
    #[test]
    fn create_forwarder() {
        let (machine, sender) = machine::create(Forwarder::new(1));
