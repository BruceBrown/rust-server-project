...
/// The ConfigMerger is passed into settings for providing custom merging of config
/// fields. After each config file is merged, the merge() method is called which
/// can perform customized config merging. As a final step in creating settings,
/// the Source is merged.
...
impl ConfigBuilder {
    ...
    /// produce it.
    pub fn build(&self, merger: &mut dyn ConfigMerger) -> Result<(ServerMetaConfig, Config), ConfigError> {
